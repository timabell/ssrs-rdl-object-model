...
namespace Reporting.ObjectModel
{
	...
	/// 
	/// </summary>
	public class BorderStyle : IXmlSerializable
	{
		...
		/// <summary>
		/// Creates a new instance of the BorderStyle class.
		/// </summary>
		public BorderStyle() { }
		...
		/// <summary>
		/// Generates an BorderStyle from its RDL representation.
		/// </summary>
		/// <param name="reader">The <typeparamref name="XmlReader"/> stream from which the BorderStyle is deserialized</param>
		public void ReadXml(XmlReader reader)
		{
			while (reader.Read())
			{
				if (reader.NodeType == XmlNodeType.EndElement && reader.Name == Rdl.BORDERSTYLE)
				{
					break;
				}
		...
		/// <summary>
		/// Converts a BorderStyle into its RDL representation .
		/// </summary>
		/// <param name="writer">The <typeparamref name="XmlWriter"/> stream to which the BorderStyle is serialized</param>
		public void WriteXml(XmlWriter writer)
		{
			//--- Style
			writer.WriteStartElement(Rdl.BORDERSTYLE);

			//--- Default
			if (_default!=null && !string.IsNullOrEmpty(_default.Value.ToString()))
				writer.WriteElementString(Rdl.DEFAULT, _default.Value.ToString());

			//--- Left
            if (_left != null && !string.IsNullOrEmpty(_left.Value.ToString()))
				writer.WriteElementString(Rdl.LEFT, _left.Value.ToString());

			//--- Right
            if (_right != null && !string.IsNullOrEmpty(_right.Value.ToString()))
				writer.WriteElementString(Rdl.RIGHT, _right.Value.ToString());

			//--- Top
            if (_top != null && !string.IsNullOrEmpty(_top.Value.ToString()))
				writer.WriteElementString(Rdl.TOP, _top.Value.ToString());

			//--- Bottom
            if (_bottom != null && !string.IsNullOrEmpty(_bottom.Value.ToString()))
